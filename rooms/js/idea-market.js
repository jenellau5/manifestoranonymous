(() => {
  ...
  function changeStatus(to, takeback = false) {
    ...
    const now = Date.now(); const updated = {
      ...i, status: to, statusUpdatedAt: now, releasedAt: to === 'RELEASED' ? now : i.releasedAt, completedAt: to === 'COMPLETED' ? now : (i.completedAt ?? null),
  ...
  window.addEventListener('pagehide', () => { $('entry-form').reset(); });
  load(); render();
})();
