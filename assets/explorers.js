(() => {
  ...
  if (channelRoot && window.MA_CHANNELS) {
    ...
    const render = () => {
      ...
      result.innerHTML = `<div class="result-kicker">CHANNEL ${esc(select.value)} · ${esc(c.circuit)}</div><h2>${esc(c.name)}</h2><div class="status-pill ${c.possible ? 'possible' : 'impossible'}">${c.possible ? 'CAN BE FULLY DEFINED IN A MANIFESTOR' : 'CANNOT BE FULLY DEFINED IN A MANIFESTOR'}</div><div class="result-grid"><div><b>CORE</b><p>${esc(c.core)}</p></div><div><b>HIGH EXPRESSION</b><p>${esc(c.high)}</p></div><div><b>SHADOW / PRESSURE</b><p>${esc(c.low)}</p></div><div><b>MANIFESTOR LENS</b><p>${esc(c.manifestor)}</p></div></div>`;
    };
    select.addEventListener('change', render); render();
  }
})();
